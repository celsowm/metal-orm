...
The `hydrateRows()` function takes an array of database rows and a hydration plan to produce nested objects. The hydration plan is generated by the query builder when you use the `includePick()` method.

```typescript
import { selectFrom, MySqlDialect, hydrateRows } from 'metal-orm';

const builder = selectFrom(users)
  .selectRaw('*')
  .includePick('posts', ['id', 'title', 'content']);
...
selectFrom(users)
  .selectRaw('*')
  .includePick('projects', ['id', 'name', 'client'], {
...
const [user] = await selectFrom(users)
  ...
  .includePick('posts', ['id', 'title']) // eager
