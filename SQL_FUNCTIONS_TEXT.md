...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **ASCII** | Returns the numeric ASCII value of the first character. | Use `unicode()` | ✔ | ✔ (In UTF-8 returns codepoint) | ✔ `ASCII()` | Native helper `ascii()` (dialect-aware). |
...
| **CHAR** | Returns a string from Unicode/ASCII integer codes. | ✔ | ✔ (Up to 1 char, 0–255) | Use `CHR` | ✔ `CHAR()` | Native helper `char(...)`. |
...
| **CHAR_LENGTH** | Returns the number of characters (not bytes). | Use `LENGTH()` | Use `LEN()` | ✔ | ✔ `CHAR_LENGTH()` | Native helper `charLength()`. |
...
| **CONCAT** | Concatenates strings (ignores NULLs in some DBs). | ✔ (v3.35+) | ✔ (SQL 2012+) | ✔ | ✔ `CONCAT()` (Returns NULL if any arg is NULL) | Native helper `concat(...args)` (SQLite emits `||`). |
| **CONCAT_WS** | Concatenates with separator (ignores NULLs). | ✔ (v3.35+) | ✔ (SQL 2017+) | ✔ | ✔ `CONCAT_WS()` | Native helper `concatWs(sep, ...)` (SQLite unsupported). |
...
| **INSTR** | Returns position of first occurrence of substring. | ✔ | Use `CHARINDEX` | Use `STRPOS` | ✔ `INSTR()` | Native helper `instr(str, sub)`. |
| **LEFT** | Extracts *N* characters from the left. | Use `SUBSTR` | ✔ | ✔ | ✔ `LEFT()` | Native helper `left(str, len)`. |
...
| **LENGTH** | Returns length of string. | ✔ (chars) | Use `LEN` | ✔ (chars) | ✔ `LENGTH()` (Returns bytes!) | Native helper `length()`. |
| **LOCATE** | Synonym for INSTR (MySQL specific syntax). | | | | ✔ `LOCATE()` | Native helper `locate(sub, str[, start])` (start unsupported on SQLite/Postgres). |
| **LOWER** | Converts to lowercase. | ✔ (ASCII only) | ✔ | ✔ | ✔ `LOWER()` | Native helper `lower()`. |
| **LPAD** | Pads string on the left. | | | ✔ | ✔ `LPAD()` | Native helper `lpad(str, len, pad)` (MySQL/Postgres). |
| **LTRIM** | Trims from the left. | ✔ | ✔ | ✔ | ✔ `LTRIM()` | Native helper `ltrim()`. |
...
| **POSITION** | Returns position of substring (Standard SQL). | ✔ | | ✔ | ✔ `POSITION()` | Native helper `position(sub, str)`. |
...
| **REPEAT** | Repeats string *N* times. | | Use `REPLICATE` | ✔ | ✔ `REPEAT()` | Native helper `repeat(str, n)` (SQL Server via `REPLICATE`, not SQLite). |
| **REPLACE** | Replaces occurrences of a substring. | ✔ | ✔ | ✔ | ✔ `REPLACE()` | Native helper `replace(str, from, to)`. |
...
| **RIGHT** | Extracts *N* characters from the right. | Use `SUBSTR` | ✔ | ✔ | ✔ `RIGHT()` | Native helper `right(str, len)`. |
| **RPAD** | Pads string on the right. | | | ✔ | ✔ `RPAD()` | Native helper `rpad(str, len, pad)` (MySQL/Postgres). |
| **RTRIM** | Trims from the right. | ✔ | ✔ | ✔ | ✔ `RTRIM()` | Native helper `rtrim()`. |
| **SOUNDEX** | Returns Soundex code. | ✔ (If enabled) | ✔ | (via extension) | ✔ `SOUNDEX()` | Emit via Metal-orm (selectRaw('SOUNDEX(...)') or a FunctionNode). |
| **SPACE** | Returns string of *N* spaces. | `repeat(' ', N)` | ✔ | `repeat(' ', N)` | ✔ `SPACE()` | Native helper `space(n)` (Postgres uses `repeat`). |
...
| **SUBSTR** | Extracts substring. | ✔ | ✔ | ✔ | ✔ `SUBSTR()` | Native helper `substr(str, start[, len])`. |
...
| **TRIM** | Trims whitespace/characters. | ✔ | ✔ (SQL 2017+) | ✔ | ✔ `TRIM()` | Native helper `trim(str[, chars])`. |
...
| **UPPER** | Converts to uppercase. | ✔ (ASCII only) | ✔ | ✔ | ✔ `UPPER()` | Native helper `upper()`. |
